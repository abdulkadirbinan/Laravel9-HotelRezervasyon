                    ...
                    <div class="table-responsive">
                      <table class="table">
                        ...
                        @foreach($data as $rs)
                          <tr>
                            <td>{{$rs->id}}</td>
                              <td><a href="{{route('admin.hotel.show',['id'=>$rs->hotel_id])}}">{{$rs->hotel->title}}</a> </td>
                            <td>{{$rs->user->name}}</td>
                              <td>{{$rs->subject}}</td>
                              <td>{{$rs->review}}</td>
                              <td>{{$rs->rate}}</td>
                              <td>{{$rs->status}}</td>
                            <td><a href="{{route('admin.comment.destroy',['id'=>$rs->id])}}" <button class="btn btn-danger"
                                 onclick="return confirm('Deleting are you sure')">Delete</a></button></td>
                            <td><a href="{{route('admin.comment.show',['id'=>$rs->id])}}" <button class="btn btn-primary"
